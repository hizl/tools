...
Я пока буду использовать такую конфигурацию, но возможно есть и другие возможности языка.(Буду смотреть в дальнейшем).
...
В книге есть подробная таблица и описание. Далее @JoinColumn(name = "details_id").
В книге которую я привел в качестве описание синтаксиса есть описание данной аннотации 
**Стр.198 Глава 5. Объектно-реляционное отображение**. Вкратце там описано, что эта аннотация служит
для настройки столбца соединения, внешнего ключа владеющей стороны. Внешний ключ называется столбцом соединения.
Реализация приведена в структуре проекта **investment.tools/Hibernate/UniDirectional** 

**@OneToOne(mappedBy = "contactDetails",cascade = CascadeType.ALL)
Dividends dividends;** Такая аннотация говорит о двухсторонней каскадной связи между двумя сущностями.
Благодаря этой аннотации обе сущности указывают друг на друга, знают друг о друге.
Вторичный ключ(foreign key) из таблицы Dividends должен ссылаться на первичный ключ(primary key)
из таблицы ContactDetails
Такая связь обеспечивает например полностью получение данных из двух таблиц каскадно, удаление же
будет выглядеть так что первая таблица ссылается на вторую, и что бы прервать эту связь для удаления потребуется
в классе занулить ссылку null - ом.

Описание получилось достаточно громоздким и большим.  